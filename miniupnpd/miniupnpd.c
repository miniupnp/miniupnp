...
if (GETFLAG(ENABLEUPNPMASK) && !GETFLAG(SECUREMODEMASK))
	syslog(LOG_WARNING, "WARNING: secure_mode=no set, allow adding port maps for non-requesting IP addresses via UPnP IGD");
#ifdef ENABLE_PCP
if (GETFLAG(ENABLENATPMPMASK) && GETFLAG(PCP_ALLOWTHIRDPARTYMASK))
	syslog(LOG_WARNING, "WARNING: pcp_allow_thirdparty=yes set, allow adding port maps for non-requesting IP addresses via PCP");
#endif
#ifdef ENABLE_IPV6
if (GETFLAG(IPV6DISABLEDMASK))
