...
#endif
			"\t-S0 disable UPnP IGD secure mode, allow adding port maps for non-requesting (other) IPs\n"
...
#endif /* V6SOCKETS_ARE_V6ONLY */
#endif /* ENABLE_HTTPS */
		syslog(LOG_NOTICE, "IPv4 enabled with public IPv4 address: %s", use_ext_ip_addr);
#ifdef ENABLE_IPV6
		if(!GETFLAG(IPV6DISABLEDMASK)) {
			if(find_ipv6_addr(lan_addrs.lh_first ? lan_addrs.lh_first->ifname : NULL,
				...
				syslog(LOG_WARNING, "No IPv6 address, disabling support");
				SETFLAG(IPV6DISABLEDMASK);
			}
		}
#endif	/* ENABLE_IPV6 */
...
if (GETFLAG(ENABLEUPNPMASK) && !GETFLAG(SECUREMODEMASK))
	syslog(LOG_WARNING, "WARNING: secure_mode=no, allow adding port maps for non-requesting (other) IPs by UPnP IGD");
#ifdef ENABLE_PCP
if (GETFLAG(ENABLENATPMPMASK) && GETFLAG(PCP_ALLOWTHIRDPARTYMASK))
	syslog(LOG_WARNING, "WARNING: pcp_allow_thirdparty=yes, allow adding port maps for non-requesting (other) IPs by PCP");
#endif
